...
void main() async {
  ...
  // Sign up a user
  final email = 'tester_audit_logs_${DateTime.now().millisecondsSinceEpoch}@ecraftz.com';
  ...
  // Query audit_logs
  final url = Uri.parse('https://vbosonyrosxfttyoengz.supabase.co/rest/v1/audit_logs?select=*');
  ...
  print('Audit logs status: ${response.statusCode}');
  try {
    final list = jsonDecode(body) as List;
    print('Row count: ${list.length}');
    if (list.isNotEmpty) {
      print('Columns: ${list.first.keys.toList()}');
    }
