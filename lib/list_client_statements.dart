...
void main() async {
  ...
  // Sign up a user
  final email = 'tester_list_cs_${DateTime.now().millisecondsSinceEpoch}@ecraftz.com';
  ...
  // Query client_statements
  final url = Uri.parse('https://bnjvugxvcoqgfvvvwpzc.supabase.co/rest/v1/client_statements?select=*');
  ...
  print('Client statements status: ${response.statusCode}');
  try {
    final list = jsonDecode(body) as List;
    print('Row count: ${list.length}');
    for (final row in list) {
      print('- ID: ${row['id']}, ClientId: ${row['client_id']}, Description: ${row['description']}');
    }
  } catch (e) {
    print('Failed: $e');
    print(body);
